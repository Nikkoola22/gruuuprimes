...
author: zevra-tech
author_url: https://github.com/zevra-tech/legal-prompt/tree/main/skills/jurisprudence
license: MIT
version: 0.1.0
execution_mode: open
jurisdiction: fr
...
## 📦 Sources et Données Massives (Open Data Judilibre)
- **API Judilibre & MCP** : Requêtage en direct des fonds Cour de cassation, Cours d'appel et Tribunaux judiciaires.
- **Dataset Open Data Hugging Face** ([`antoinejeannot/jurisprudence`](https://huggingface.co/datasets/antoinejeannot/jurisprudence)) :
  - Plus de 1,12 million de décisions judiciaires consolidées (Cour de cassation depuis 1860, Cour d'appel, TJ).
  - Synchronisé toutes les 72h via l'API Judilibre officielle.
  - Formats exploitables directement : Parquet & JSONL (`scan_parquet` DuckDB/Polars ou librairie `datasets`).
- **CLI local** : [`scripts/query_jurisprudence.py`](../../scripts/query_jurisprudence.py) — requêtage ciblé des Parquets distants via DuckDB (par pourvoi, dates, chambre, mots-clés, publication au Bulletin). Dépendances autonomes via [`requirements.txt`](../../requirements.txt) : `python3 -m venv .venv && .venv/bin/pip install -r requirements.txt`, puis `.venv/bin/python scripts/query_jurisprudence.py -j cassation -q "proportionnalité sanction" -l 3`.
