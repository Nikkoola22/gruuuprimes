...
### Outils & APIs d'Accès aux Données Juridiques (Support Technique)

| Outil / Dépôt | Type | Usage & Accès |
|---------------|------|---------------|
| `droit-francais-mcp` | Serveur MCP local | Interrogation directe API Légifrance & JudiLibre (PISTE) |
| `rdassignies/pylegifrance` & `mcp-server-legifrance` | SDK / MCP Python | Client Python typé Pydantic pour requêtes automatisées |
| `SocialGouv/legi-data` | API / JSON | Formats JSON structurés des codes français |
| `legalize-dev/legalize-fr` | Git / Markdown | Versioning historique article par article |
| `OpenDataFrance` / `etalab/schema.data.gouv.fr` | Schémas territoriaux | Référentiels d'arrêtés, délibérations et marchés publics |
