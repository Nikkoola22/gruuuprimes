...
export const ArrTeletravail: React.FC = () => {
  ...
  // 4. TELETRAVAIL
  const [joursParSemaine, setJoursParSemaine] = useState("1");
  const [joursFixes, setJoursFixes] = useState("");
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fadeIn pb-12">
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200 dark:border-slate-700">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">Arrêté de Télétravail</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">Autorisation d'exercice en télétravail</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-5 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-700">
            <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-4 flex items-center gap-2">
              <Calendar className="w-4 h-4 text-slate-400" /> Modalités du télétravail
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Nombre de jours par semaine</label>
                <select value={joursParSemaine} onChange={(e) => handleInputChange(setJoursParSemaine, e.target.value)} className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white">
                  <option value="1">1 jour</option>
                  <option value="2">2 jours</option>
                  <option value="3">3 jours (maximum réglementaire)</option>
                </select>
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Jours fixés (ex: Mardi et Jeudi)</label>
                <input type="text" value={joursFixes} onChange={(e) => handleInputChange(setJoursFixes, e.target.value)} placeholder="Ex: Mardi" className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="date" value={dateFin} onChange={(e) => handleInputChange(setDateFin, e.target.value)} className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Lieu d'exercice</label>
                <input type="text" value={lieu} onChange={(e) => handleInputChange(setLieu, e.target.value)} className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white" />
              </div>
          ...
          <div className="border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm bg-white dark:bg-slate-900">
            ...
            <div className="p-10 font-serif text-[13px] leading-relaxed text-slate-900 dark:text-slate-200 max-h-[600px] overflow-y-auto" id="arrete-content">
              ...
              <h4 className="text-center font-bold text-lg mb-8 uppercase">
                ARRÊTÉ PORTANT AUTORISATION DE TÉLÉTRAVAIL<br/>
                POUR {civilite.toUpperCase()} {renderField(nom.toUpperCase(), "NOM")} {renderField(prenom, "PRÉNOM")}
              </h4>
              ...
              <div className="space-y-4 text-justify">
                ...
                <p><strong>Vu</strong> le décret n° 2016-151 du 11 février 2016 relatif aux conditions et modalités de mise en œuvre du télétravail dans la fonction publique et la magistrature,</p>
                <p><strong>Vu</strong> la délibération du Conseil municipal en date du ............................ fixant les modalités d'exercice du télétravail,</p>
                <p><strong>Vu</strong> la demande écrite formulée par {civilite} {renderField(nom.toUpperCase(), "NOM")} {renderField(prenom, "PRÉNOM")}, {renderField(grade, "GRADE")}, sollicitant l'autorisation d'exercer ses fonctions en télétravail,</p>
                
                <div className="text-center font-bold mt-8 mb-6">ARRÊTE</div>

                <p><strong>ARTICLE 1 :</strong><br/>
                À compter du {renderField(dateDebut, "DATE DE DÉBUT")}, {civilite} {renderField(nom.toUpperCase(), "NOM")} {renderField(prenom, "PRÉNOM")} est autorisé(e) à exercer une partie de ses fonctions en télétravail à son {renderField(lieu, "LIEU")}.</p>

                <p><strong>ARTICLE 2 :</strong><br/>
                L'autorisation est accordée pour une durée allant jusqu'au {renderField(dateFin, "DATE DE FIN")}. La quotité de télétravail est fixée à {renderField(joursParSemaine, "X")} jour(s) par semaine, organisée comme suit : {renderField(joursFixes, "JOURS FIXÉS")}.</p>
