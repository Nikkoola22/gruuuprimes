...
import { FileSignature, Copy, CheckCircle2, User, Building, Info, RefreshCw } from "lucide-react";
...
export const ArrMutationInterne: React.FC = () => {
  ...
  // 3. MUTATION INTERNE (GRADE UNIQUE)
  ...
  const [ancienService, setAncienService] = useState("");
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fadeIn pb-12">
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200 dark:border-slate-700">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">Arrêté de Mutation Interne</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">Changement de service ou de poste en interne</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-5 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-700">
            <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-4 flex items-center gap-2">
              <Building className="w-4 h-4 text-slate-400" /> Collectivité
            </h3>
          ...
          <div className="p-5 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-700">
            <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-4 flex items-center gap-2">
              <RefreshCw className="w-4 h-4 text-slate-400" /> Détails de l'affectation
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Ancien service d'affectation</label>
                <input type="text" value={ancienService} onChange={(e) => handleInputChange(setAncienService, e.target.value)} placeholder="Ex: Service Jeunesse" className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Nouveau service d'affectation</label>
                <input type="text" value={nouveauService} onChange={(e) => handleInputChange(setNouveauService, e.target.value)} placeholder="Ex: Service État Civil" className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white" />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Intitulé du nouveau poste (facultatif)</label>
                <input type="text" value={intitulePoste} onChange={(e) => handleInputChange(setIntitulePoste, e.target.value)} placeholder="Ex: Responsable des mariages" className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white" />
              </div>
            </div>
          ...
          <div className="border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm bg-white dark:bg-slate-900">
            ...
            <div className="p-10 font-serif text-[13px] leading-relaxed text-slate-900 dark:text-slate-200 max-h-[600px] overflow-y-auto" id="arrete-content">
              ...
              <h4 className="text-center font-bold text-lg mb-8 uppercase">
                ARRÊTÉ PORTANT MUTATION INTERNE<br/>
              ...
              <div className="space-y-4 text-justify">
                ...
                <p><strong>Considérant</strong> les nécessités de service requérant le redéploiement de {civilite} {renderField(nom.toUpperCase(), "NOM")} {renderField(prenom, "PRÉNOM")} dans un nouveau service,</p>
                
                <div className="text-center font-bold mt-8 mb-6">ARRÊTE</div>

                <p><strong>ARTICLE 1 :</strong><br/>
                À compter du {renderField(dateEffet, "DATE D'EFFET")}, {civilite} {renderField(nom.toUpperCase(), "NOM")} {renderField(prenom, "PRÉNOM")}, affecté(e) jusqu'alors au sein de {renderField(ancienService, "ANCIEN SERVICE")}, est affecté(e) au sein de {renderField(nouveauService, "NOUVEAU SERVICE")}{intitulePoste ? ` en qualité de ${renderField(intitulePoste, "INTITULÉ DU POSTE")}` : ""}.</p>

                <p><strong>ARTICLE 2 :</strong><br/>
                Cette affectation n'emporte aucune modification de la situation statutaire de l'agent qui conserve son grade de {renderField(grade, "GRADE")} ainsi que son classement indiciaire actuel.</p>
